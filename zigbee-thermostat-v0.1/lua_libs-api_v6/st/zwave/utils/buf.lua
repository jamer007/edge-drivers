...
--- Find the precision of a decimal number. By default, we pack the number into
--- as few bytes as possible (e.q 21.5000000 with precision 7 would be truncated to
--- 21.5 with precision 1). However, if a size override is provided, then
...
--- values with a 2's complement significand of 'size' bytes.  The maximum negated
...
local function _precision(value, size)
  ...
  local precision = 0
  -- If no size is provided, we default to finding minimum precision
  -- so that float can be packed in as few bytes as possible. We will
  -- determine how much can be truncated after calculating the width
  -- of the non-fractional component of the number below.
  local truncate = size == nil

  size = size or FLOAT_BYTES
  local clrsb = size * 8 - _width(value)
  precision = math.min(7, math.floor(math.log(1 << clrsb, 10)))

  if truncate and precision > 0 then
    local truncated_precision = 0
    while math.abs(math.fmod(value,1)) ~= 0 and truncated_precision < precision do
      value = value * 10
      truncated_precision = truncated_precision + 1
    end
    precision = truncated_precision
  end
  return precision
end
...
function Writer.size(value)
  ...
  if _type(value) == "float" then
    -- Convert the float type to a whole number represenation to determine size.
    -- Floats are represented with special handling and by default we pack floats into
    -- as few bytes as possible. Floats are represented by a signed int value and a corresponding
    -- precision such that float = value * 10 ^ -precision. For example, for
    -- value 21.5000000, this would be truncated to 21.5 by using value = 215 and precision = 1
    -- so that 215 * 10^-1 = 21.5. This way, the value can be represented by 2 bytes (215) rather
    -- than 4 bytes (215000000).
    value = math.floor(value * 10^_precision(value))
  end
  if _type(value) == "integer" then
