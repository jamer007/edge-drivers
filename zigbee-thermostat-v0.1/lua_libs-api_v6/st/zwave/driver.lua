...
local function extend_wakeup_handler(driver)
  ...
  if driver.zwave_handlers[cc.WAKE_UP] ~= nil then
    ...
    driver.zwave_handlers[cc.WAKE_UP] = {}
  end
  ...
  --Once per driver restart when a sleepy device wakes up, we should check the
  --wake up interval for the hubs device health impl to function well for migrated devices.
  local wakeup_interval_get = function(inner_driver, device, cmd)
    if not device:get_field(constants.WAKEUP_INTERVAL_GET_SENT) then
      device:send(WakeUp:IntervalGetV1({}))
      device:set_field(constants.WAKEUP_INTERVAL_GET_SENT, true)
    end
  end
  if existing_wakeup_notification_handler == nil then
    driver.zwave_handlers[cc.WAKE_UP][WakeUp.NOTIFICATION] = {wakeup_interval_get}
  elseif type(existing_wakeup_notification_handler) == "table" then
    table.insert(driver.zwave_handlers[cc.WAKE_UP][WakeUp.NOTIFICATION], 1, wakeup_interval_get)
  elseif type(existing_wakeup_notification_handler) == "function" then
    driver.zwave_handlers[cc.WAKE_UP][WakeUp.NOTIFICATION] = {wakeup_interval_get, existing_wakeup_notification_handler}
  end
