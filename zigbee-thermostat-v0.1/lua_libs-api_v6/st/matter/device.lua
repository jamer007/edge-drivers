...
---
--- @param cluster_id number|nil the cluster ID to check for (if nil, all endpoint ids are returned)
...
function MatterDevice:get_endpoints(cluster_id, opts)
  ...
  for _, ep in ipairs(self.endpoints) do
    for _, clus in ipairs(ep.clusters) do
      if (clus.cluster_id == cluster_id and (opts.feature_bitmap == nil or clus_has_features(clus, opts.feature_bitmap)))
...
--- single component device does not have endpoint id 1
...
local function find_default_endpoint(device, component)
  local res = matter_device.MATTER_DEFAULT_ENDPOINT
  if utils.table_size(device.profile.components) == 1 then
...
function MatterDevice:component_to_endpoint(comp_id)
  ...
  if comp_to_ep ~= nil then
    ...
    return find_default_endpoint(self, comp_id)
  end
end
...
function MatterDevice:subscribe()
  populate_subscribed_attributes_from_driver_defaults(self, self.driver.subscribed_attributes, self.driver.subscribed_events)
...
function MatterDevice:send(req)
  if type(req) == "table" and getmetatable(req) == im.InteractionRequest then
    self.log.info_with({hub_logs = true}, string.format("sending InteractionRequest: %s", req))
...
function MatterDevice.init(cls, driver, raw_device)
    ...
    out_device = base_device.Device._protect(cls, out_device)
