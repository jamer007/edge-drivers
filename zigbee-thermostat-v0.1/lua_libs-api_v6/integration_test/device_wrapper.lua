...
local base_protect = base_device.Device._protect
...
local function wrapped_protect(cls, device)
  -- device doesn't have the id field at this point 
  -- so finding the mock device is a little convoluted
  local mock_dev
  for _, v in pairs(wrapped_device_mod.mock_devices) do
    local wrapped_dev = rawget(v, "wrapped_device")
    if wrapped_dev == device then
      mock_dev = v
      break
    end
  end
  device = base_protect(cls, device)
  rawset(mock_dev, "wrapped_device", device)
  return device
end

base_device.Device.init = wrapped_init
base_device.Device._protect = wrapped_protect
