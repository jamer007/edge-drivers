...
function MockCapabilityChannel:get_capability_definition(capability_id, capability_version)
  ...
  if cap_def ~= nil then
    ...
    local req_path = string.format("st.capabilities.generated.%s", capability_id)
    local status, cap_val = pcall(require, req_path)
    if status then
      local cap_json = capabilities.build_cap_from_json_string(cap_val)
      self.__load_capability_definition(capability_id, capability_version, cap_json)
      return cap_val
    else
